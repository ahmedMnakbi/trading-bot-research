...
class DonchianBreakoutStrategy:
    ...
    @classmethod
    def from_params(cls, params: dict[str, object]) -> DonchianBreakoutStrategy:
        return cls(
            donchian_lookback=int(params.get("donchian_lookback", 20) or 20),
    ...
    def generate_signal(
        ...
        min_rows = max(self.donchian_lookback + 1, self.atr_period)
        if len(candles) < min_rows:
            return TradeIntent.hold("insufficient_history")
        channel = donchian_channel(candles, self.donchian_lookback)
        ...
        previous_middle = channel.middle.iloc[-2]
        current_atr = atr.iloc[-1]
        if pd.isna(previous_upper) or pd.isna(previous_middle) or pd.isna(current_atr):
            return TradeIntent.hold("indicator_unavailable")
        if account.position is not None:
            if current_close < float(previous_middle):
                return TradeIntent(action="EXIT", reason="strategy_exit")
            return TradeIntent.hold("in_position")
        if current_close <= float(previous_upper) or current_atr <= 0:
            return TradeIntent.hold("no_breakout")
        ...
        return TradeIntent(
            ...
            metadata={"previous_upper": float(previous_upper), "atr": float(current_atr)},
        )
